	...
	public boolean createTutor()
	{
		...
		try {
			...
			try {
				...
				if (generatedKeys.next()) {profile_id = generatedKeys.getLong(1);flag = true;}
...
public boolean validate()
{
   ...
   // String pattern1= "^.*[a-zA-Z0-9]+.*$";
    ...
    String pattern4="^([0,1]{7})$";
   // int len2=dayslot.length();
    
	if(name!="" && address!="" &&  email!="" && dob!="" && contact!="" && discipline!="" && subdiscipline!="" && dayslot!="" && timeslot!="" && location!="" && description!="")
	{
		 if(p>0 && d>0 && d>p)
		 {
			 if(name.matches(pattern2) && contact.matches(pattern3) && dayslot.matches(pattern4))
			 {
				 valid=true;
			 }
		}
