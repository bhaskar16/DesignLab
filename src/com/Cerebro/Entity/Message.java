...
public class Message {
	...
	public Message(long cId, String msg, String dir) {
		...
		try {
			...
			if(cnt == 1) {
				...
				while(rs.next()){dt = sdf.format(rs.getTimestamp("date"));}
	...
	public Message(Message m) {
		db_id = m.getId();contract_id = m.getContractId();message = m.getMessage();
	...
	{
		if(direction.equals("s-t"))
				return "Student says";
		else if(direction.equals("t-s"))
				return "Tutor says";
		else 
				return null;
	}
